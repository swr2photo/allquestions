#!/usr/bin/env node
import { createRequire as __banner_cR } from 'module'; const require = __banner_cR(import.meta.url);
...
var AuthManager = class {
  ...
  async runAuthentication(onStatus) {
    ...
    try {
      await Promise.race([
        ...
        page.waitForURL(
          (url) => url.origin === "https://notebooklm.google.com" && (url.pathname.includes("/notebook") || url.pathname === "/"),
          { timeout: 3e5 }
        ).then(() => {
          isDone = true;
        }),
        // 2. Main app structure (main role, notebook grid, or aria-labels)
        page.waitForSelector('div[role="main"], .notebook-grid, [aria-label*="Notebook"], [aria-label*="notebook"]', { timeout: 3e5 }).then(() => {
          isDone = true;
        }),
        // 3. Account indicators (profile pic, logout links)
        page.waitForSelector('button[aria-haspopup="true"] img[src*="googleusercontent.com"], a[href*="logout"], a[href*="Logout"]', { timeout: 3e5 }).then(() => {
          isDone = true;
        }),
          ...
          const checkLoop = async () => {
            ...
            try {
              ...
              if (sid && page.url().includes("notebooklm.google.com")) {
                ...
                if (!isDone) setTimeout(checkLoop, 2e3);
              }
            } catch (e) {
              isDone = true;
            }
    ...
    const cookieString = Array.from(cookieMap.entries()).map(([name, value]) => `${name}=${value}`).join("; ");
...
async function runAuthCli() {
  ...
  console.error(import_chalk.default.cyan.bold("\u2551      NotebookLM MCP Authentication        \u2551"));
  console.error(import_chalk.default.cyan.bold("\u255A\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255D\n"));
  try {
    await auth.runAuthentication((status) => {
      console.error(import_chalk.default.blue(`[Status] ${status}`));
    });
    console.error("\n" + import_chalk.default.green.bold("Authentication successful!"));
    console.error(import_chalk.default.white("Your session is now active. You can close any leftovers and return to your chat."));
  } catch (error) {
    console.error("\n" + import_chalk.default.red.bold("Authentication failed"));
