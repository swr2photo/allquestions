...
setInterval(updateCountdowns, 1000);
initializeSubjects();
// ฟังก์ชันเปิด Popup พร้อมข้อความและรูปภาพ
...
// เรียกใช้งาน Popup เมื่อโหลดหน้าเว็บ
document.addEventListener('DOMContentLoaded', () => {
  const announcementMessage = "ปิดปรับปรุงเว็บไซต์ในวันที่ 25 ธันวาคม 2024 เวลา 23:59 น.";
  const announcementImage = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFpyWfgsKD9NnFIF9lBqGwL2qXU183rLrdgQ&usqp=CAU"; // URL รูปภาพ
  showPopup(announcementMessage, announcementImage);
});
    
  </script>
  <script>
    // ดึง header.html มาแสดง
    fetch('header.html')
      .then(response => response.text())
      .then(data => {
        document.getElementById('header-container').innerHTML = data;
      })
      .catch(error => console.error('ไม่สามารถโหลด header ได้:', error));
  </script>
